...
def evalList(ast,message,dict):
    if ast == []:
        ...
        if message == 'bad':
            raise Exception('CDR of empty list error!')
        else:
            return head[1]
    elif ast[0] == 'CONS':
  ...
  while True:
    ...
    try:
      ...
      if ast[0] == 'list':
          ...
          expression = getIdentifiers(expression,[])
          print(
             val+'(' + str(expression[0]) + ')' if len(expression) == 1 else val+
             str(tuple(expression))
          )
      else:
          print(val+str(evalNum(ast[1],'good',{})))
